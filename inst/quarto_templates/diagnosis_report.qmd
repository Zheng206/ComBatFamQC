...
if (!is.null(params$data$mdmr.summary)) {
  mdmr <- as_tibble(params$data$mdmr.summary) %>% mutate(across(everything(), ~ if (is.list(.)) unlist(.) else .)) %>% mutate(sig = ifelse(is.na(sig), "", sig))
...
```{r}
anova_test_df <- as_tibble(params$data$anova_test_df) %>% mutate(across(everything(), ~ if (is.list(.)) unlist(.) else .)) %>% mutate(sig = ifelse(is.na(sig), "", sig))
...
if (!is.null(params$data$anova_test_df)) {
  pct <- 100 * (n - sum(anova_test_df$sig == ""))/n
...
```{r}
kw_test_df <- as_tibble(params$data$kw_test_df) %>% mutate(across(everything(), ~ if (is.list(.)) unlist(.) else .)) %>% mutate(sig = ifelse(is.na(sig), "", sig))
n <- nrow(kw_test_df)
if (n != 0 ) {
...
if (!is.null(params$data$kw_test_df)) {
  pct <- 100 * (n - sum(kw_test_df$sig == ""))/n
  cat(paste0("The percentage of significant features is: ", round(pct, 2), "."))
}
```
...
n <- nrow(kr_test_df)
if (n != 0) {
  kr_test_df %>% mutate(sig = ifelse(is.na(sig), "", sig)) %>% dplyr::select(.data[["feature"]], .data[["p.value"]], .data[["sig"]]) %>% datatable(options = list(columnDefs = list(list(className = 'dt-center', targets = "_all")))) %>% formatStyle(columns = c("p.value"),color = styleEqual(unlist(params$data$red), "red")) %>% formatStyle(
          'sig',
          target = 'row',
          backgroundColor = styleEqual(c("*", "**", "***"), "lightyellow"))
} else {
...
```{r}
if (n != 0) {
  pct <- 100 * (n - sum(kr_test_df$sig == ""))/n
  cat(paste0("The percentage of significant features is: ", round(pct, 2), "."))
}
```
:::
...
```{r}
lv_test_df <- as_tibble(params$data$lv_test_df) %>% mutate(across(everything(), ~ if (is.list(.)) unlist(.) else .)) %>% mutate(sig = ifelse(is.na(sig), "", sig))
n <- nrow(lv_test_df)
if (n != 0) {
...
if (!is.null(params$data$lv_test_df)) {
  pct <- 100 * (n - sum(lv_test_df$sig == ""))/n
  cat(paste0("The percentage of significant features is: ", round(pct, 2), "."))
}
```
...
```{r}
bl_test_df <- as_tibble(params$data$bl_test_df) %>% mutate(across(everything(), ~ if (is.list(.)) unlist(.) else .)) %>% mutate(sig = ifelse(is.na(sig), "", sig))
n <- nrow(bl_test_df)
if (n != 0 ) {
...
if (!is.null(params$data$bl_test_df)) {
  pct <- 100 * (n - sum(bl_test_df$sig == ""))/n
  cat(paste0("The percentage of significant features is: ", round(pct, 2), "."))
}
```
...
```{r}
fk_test_df <- as_tibble(params$data$fk_test_df) %>% mutate(across(everything(), ~ if (is.list(.)) unlist(.) else .)) %>% mutate(sig = ifelse(is.na(sig), "", sig))
n <- nrow(fk_test_df)
if (n != 0) {
...
if (!is.null(params$data$fk_test_df)) {
  pct <- 100 * (n - sum(fk_test_df$sig == ""))/n
  cat(paste0("The percentage of significant features is: ", round(pct, 2), "."))
}
```
:::
